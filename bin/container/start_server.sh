...
if docker ps -a -f name="$contNameServer" | grep "$contNameServer" > /dev/null; then
    ...
    if [ -z "$toHostPort" ]; then
        docker run -d --name "$contNameServer" --cpuset-cpus=0-2 -v ${extConfDir}:/opt/extra-conf-enabled -v ${extSitesConfDir}:/opt/extra-sites-enabled -v ${extDataDir}:/opt/extra-data "$imageName"
    else
        echo "map container port 80 -> host port $toHostPort"
        docker run -d --name "$contNameServer" --cpuset-cpus=0-2 -p $toHostPort:80 -v ${extConfDir}:/opt/extra-conf-enabled -v ${extSitesConfDir}:/opt/extra-sites-enabled -v ${extDataDir}:/opt/extra-data "$imageName" 
    fi
fi
